...
$hasScssChanges = $false
$scssMatches = $modifiedFiles | Where-Object { $_ -match "\.scss$" -and $_ -match "src/main/frontend/sass" }
if ($scssMatches.Count -gt 0) {
    $hasScssChanges = $true
    Write-Host "`n[SCSS] Detected SCSS file changes:" -ForegroundColor Magenta
    foreach ($scssFile in $scssMatches) {
        Write-Host "  - $scssFile" -ForegroundColor Magenta
    }
}

# Auto-build CSS if SCSS files were modified
if ($hasScssChanges) {
    Write-Host "`n[BUILD] SCSS changes detected. Building CSS files..." -ForegroundColor Yellow
    
    $frontendPath = Join-Path (Get-Location) "src\main\frontend"
    if (Test-Path $frontendPath) {
        $originalLocation = Get-Location
        Set-Location $frontendPath
        
        try {
            # Check if node_modules exists (grunt installed)
            if (-not (Test-Path "node_modules")) {
                Write-Host "[BUILD] Installing npm dependencies..." -ForegroundColor Gray
                npm install 2>&1 | Out-Null
            }
            
            # Find grunt executable
            $gruntCmd = $null
            $gruntCmdPath = Join-Path $frontendPath "node_modules\.bin\grunt.cmd"
            if (Test-Path $gruntCmdPath) {
                $gruntCmd = $gruntCmdPath
            } elseif (Get-Command grunt -ErrorAction SilentlyContinue) {
                $gruntCmd = "grunt"
            } else {
                Write-Host "[ERROR] Grunt not found. Please install: npm install" -ForegroundColor Red
                throw "Grunt not found"
            }
            
            # Run grunt sass
            Write-Host "[BUILD] Running 'grunt sass'..." -ForegroundColor Gray
            $sassOutput = & $gruntCmd sass 2>&1
            if ($LASTEXITCODE -ne 0) {
                Write-Host "[WARN] 'grunt sass' failed with exit code $LASTEXITCODE" -ForegroundColor Yellow
                $sassOutput | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
            } else {
                Write-Host "[BUILD] ✓ 'grunt sass' completed" -ForegroundColor Green
            }
            
            # Run grunt concat:deploy_files to create dist/css/style.css
            Write-Host "[BUILD] Running 'grunt concat:deploy_files'..." -ForegroundColor Gray
            $concatOutput = & $gruntCmd concat:deploy_files 2>&1
            if ($LASTEXITCODE -ne 0) {
                Write-Host "[WARN] 'grunt concat:deploy_files' failed with exit code $LASTEXITCODE" -ForegroundColor Yellow
                $concatOutput | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
            } else {
                Write-Host "[BUILD] ✓ 'grunt concat:deploy_files' completed" -ForegroundColor Green
            }
            
            # Verify style.css was created
            $styleCssPath = Join-Path $frontendPath "dist\css\style.css"
            if (Test-Path $styleCssPath) {
                Write-Host "[BUILD] style.css generated successfully" -ForegroundColor Green
                # Add style.css to modified files list for patching
                $modifiedFiles = $modifiedFiles + @("src/main/frontend/dist/css/style.css")
                Write-Host "[BUILD] Added style.css to patch files" -ForegroundColor Cyan
            } else {
                Write-Host "[WARN] style.css not found after build at: $styleCssPath" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "[ERROR] Failed to build CSS: $_" -ForegroundColor Red
            Write-Host "[WARN] Continuing without CSS build. Please run 'grunt sass' and 'grunt concat' manually." -ForegroundColor Yellow
        } finally {
...
$classAdded = 0
$jsAdded = 0
$hasComponentFile = $false  # component 관련 파일이 있는지 체크
$copiedFiles = @()  # 복사된 파일 목록 저장
...
foreach ($file in $modifiedFiles) {
    ...
    # Path mapping
    if ($file -match "^src/main/frontend/dist/(.+)$") {
        # Grunt build output files (JS, CSS from dist folder)
        $relativePath = $Matches[1] -replace "/", "\"
        $patchPath = "resources/$($Matches[1])"
        $actualFilePath = "src\main\frontend\dist\$relativePath"
        
        if ($file -match "\.js$") {
            $isJsFile = $true
            # component 관련 파일인지 체크
            if ($Matches[1] -match "^app/dashboard/component/") {
                $hasComponentFile = $true
            }
        } elseif ($file -match "\.css$") {
            # CSS file from dist (built from SCSS)
            $isCssFile = $true
        }
    } elseif ($file -match "^src/main/frontend/src/(.+)$") {
        # JavaScript files - use Grunt build output from dist folder
        $isJsFile = $true
        $relativePath = $Matches[1] -replace "/", "\"
        $patchPath = "resources/$($Matches[1])"
        $actualFilePath = "src\main\frontend\dist\$relativePath"
    ...
    if ($patchPath) {
        ...
        if (Test-Path $actualFilePath) {
            ...
            if ($isJavaFile) {
                ...
                Write-Host "  [CSS] $actualFilePath (built from SCSS)" -ForegroundColor Magenta
            } else {
            ...
            if ($isJavaFile) {
                ...
                Write-Host "  [SKIP] $actualFilePath (not found, run 'grunt sass' and 'grunt concat')" -ForegroundColor Yellow
            } else {
...
# component 관련 파일이 있으면 components.js도 자동 추가
if ($hasComponentFile) {
    $componentsJsPath = "src\main\frontend\dist\app\dashboard\components.js"
    $componentsJsPatchPath = "resources/app/dashboard/components.js"
    
    if (Test-Path $componentsJsPath) {
        $destPath = Join-Path $patchRoot $componentsJsPatchPath
        $destDir = Split-Path -Parent $destPath
        if (-not (Test-Path $destDir)) {
            New-Item -Path $destDir -ItemType Directory -Force | Out-Null
        }
        
        Copy-Item -Path $componentsJsPath -Destination $destPath -Force
        # 복사된 파일 목록에 추가
        $copiedFiles += $componentsJsPatchPath
        Write-Host "  [JS] $componentsJsPath (auto-added: component file detected)" -ForegroundColor Magenta
        $jsAdded++
        $copied++
    } else {
        Write-Host "  [SKIP] $componentsJsPath (not found, run 'grunt build')" -ForegroundColor Yellow
    }
}
...
# English labels for README
$modeText = if ($UseCommits) { "Commit Selection" } else { "Modified Files" }
$guideTitle = "Partial Patch Guide"
$patchNameLabel = "Patch Name"
$dateLabel = "Date"
$branchLabel = "Branch"
$modeLabel = "Mode"
$descLabel = "Description"
$fileLabel = "Files"
$fileCountLabel = "Total"
$fileUnit = "files"
$fileListTitle = "Included Files"
$applyMethodTitle = "How to Apply"
$cautionTitle = "Cautions"

# README 내용을 StringBuilder로 구성
$sb = New-Object System.Text.StringBuilder

# 헤더
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("  $guideTitle") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("${patchNameLabel}: $PatchName") | Out-Null
$sb.AppendLine("${dateLabel}: $date") | Out-Null
$sb.AppendLine("${branchLabel}: $branch") | Out-Null
$sb.AppendLine("${modeLabel}: $modeText") | Out-Null
$sb.AppendLine("${descLabel}: $Description") | Out-Null
$sb.AppendLine("") | Out-Null
$cssAdded = ($copiedFiles | Where-Object { $_ -match "\.css$" }).Count
$sb.AppendLine("${fileLabel}: ${fileCountLabel} $copied ${fileUnit} ($classAdded .class ${fileUnit}, $jsAdded Grunt build .js ${fileUnit}, $cssAdded .css ${fileUnit})") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("  $fileListTitle") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("") | Out-Null

# 파일 목록 추가 (정렬하여 가독성 향상)
if ($copiedFiles.Count -gt 0) {
    $sortedFiles = $copiedFiles | Sort-Object
    foreach ($file in $sortedFiles) {
        $sb.AppendLine("  - $file") | Out-Null
    }
} else {
    $sb.AppendLine("  (No files included)") | Out-Null
}

# 적용 방법
$sb.AppendLine("") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("  $applyMethodTitle") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("1. Stop Tomcat Server") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("2. Backup ROOT folder") | Out-Null
$sb.AppendLine("   tar -czf ROOT_backup_`$(date +%Y%m%d_%H%M%S).tar.gz -C [TOMCAT_HOME]/webapps ROOT") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("3. Extract and apply patch files to Tomcat webapps/ROOT/") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("4. Restart Tomcat Server") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("5. Clear browser cache (Alt+Shift+R)") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("  $cautionTitle") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("- Backup original files before applying patch") | Out-Null
$sb.AppendLine("- Test in development environment first") | Out-Null
$sb.AppendLine("- Verify Tomcat logs after restart") | Out-Null
$sb.AppendLine("") | Out-Null
$sb.AppendLine("===============================================") | Out-Null
...
try {
    ...
    if ($gitRoot -and (Test-Path $gitRoot)) {
        ...
        Write-Host "[INFO] Changed to Git repository root: $gitRoot" -ForegroundColor Gray
    }
} catch {
    Write-Host "[WARN] Could not determine Git repository root, using current directory" -ForegroundColor Yellow
}

# Get absolute path for README file
$readmeFullPath = Join-Path (Get-Location) "PATCH_README.txt"

# UTF-8 BOM으로 저장
$utf8WithBom = New-Object System.Text.UTF8Encoding $true
[System.IO.File]::WriteAllText($readmeFullPath, $sb.ToString(), $utf8WithBom)

Write-Host "`n[README] PATCH_README.txt created at: $readmeFullPath" -ForegroundColor Cyan

# Verify README file was created
if (-not (Test-Path $readmeFullPath)) {
    Write-Host "[ERROR] Failed to create PATCH_README.txt at: $readmeFullPath" -ForegroundColor Red
    exit 1
}

# Copy to Downloads folder (www folder with ROOT inside, and README at same level)
$downloadsPath = [Environment]::GetFolderPath('UserProfile') + "\Downloads\TACEA_$PatchName"
...
if (Test-Path "www") {
    ...
    Write-Host "[WARN] www folder not found at: $(Get-Location)\www" -ForegroundColor Yellow
}

# Copy README file
Copy-Item -Path $readmeFullPath -Destination $downloadsPath -Force
Write-Host "[COPY] $downloadsPath" -ForegroundColor Yellow

# Create ZIP with proper directory entries
$zipPath = [Environment]::GetFolderPath('UserProfile') + "\Downloads\TACEA_$PatchName.zip"
...
try {
    # Add README file at root level (verify file exists first)
    if (Test-Path $readmeFullPath) {
        ...
        Write-Host "[ERROR] PATCH_README.txt not found at: $readmeFullPath" -ForegroundColor Red
        throw "PATCH_README.txt file not found"
    }
...
$cssCount = ($copiedFiles | Where-Object { $_ -match "\.css$" }).Count
if ($cssCount -gt 0) {
    Write-Host ".css files: $cssCount" -ForegroundColor Magenta
}
